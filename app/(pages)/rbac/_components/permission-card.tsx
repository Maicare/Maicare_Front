...
// Transformeer de gegeven data naar gegroepeerde permissies
const transformPermissions = (data: {
    ...
    data.forEach(item => {
        // Haal categorie op uit permission_name (bijv. "TEST" van "TEST.VIEW")
...
const PermissionCard = ({ selectedRole,selectedUser }: { selectedRole: Role | null , selectedUser: Id | null }) => {
    ...
    useEffect(()=>{
        const getPermissions = async () => {
            try {
                ...
                console.error("Fout bij ophalen permissies:", error);
            }
        };
        getPermissions();
    ...
    useEffect(() => {
        const getRolePermissions = async () => {
            ...
            try {
                ...
                console.error("Fout bij ophalen rol permissies:", error);
            }
        };
        const getEmployeePermissions = async () => {
            ...
            try {
                ...
                console.error("Fout bij ophalen medewerker permissies:", error);
            }
        };
        if (selectedRole) {
            getRolePermissions();
    ...
    const saveRolePermissions = async() => {
        ...
        try {
            ...
            console.error("Fout bij opslaan rol permissies:", error);
        }
    };
            ...
            <Card className="bg-white border-indigo-100 shadow-sm h-full">
                <CardHeader className="bg-gradient-to-r from-teal-500 to-cyan-600 text-white rounded-t-lg py-4">
                    <CardTitle className="flex items-center">
                        ...
                        Permissies voor {selectedRole?.role_name || 'Selecteer een rol'}
                    </CardTitle>
                    <CardDescription className="text-cyan-100">
                        Beheer permissies voor de geselecteerde rol
                    </CardDescription>
                </CardHeader>
                <CardContent className="p-4  max-h-[900px] overflow-y-scroll">
                    {selectedRole || selectedUser ? (
                        ...
                        <div className="text-center py-12 text-gray-500">
                            ...
                            <p className="mt-4">Selecteer een rol om de permissies te beheren</p>
                        </div>
                    )}
                </CardContent>
            </Card>
