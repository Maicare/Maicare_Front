...
import { CreateAppointment, appointmentSchema } from "@/schemas/appointmentSchema";
...
const BookingPopup: FunctionComponent<BookingPopupProps> = ({
  ...
  const { CreateBooking, UpdateBooking, DeleteBooking } = useAppointment();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const createdOrUpdated = editEvent
        ? await UpdateBooking(editEvent.event.id, parsed.data)
        : await CreateBooking(parsed.data); const fg = getContrastColor(selectedColor);
