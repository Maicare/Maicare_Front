...
export function UpdateInvoiceForm({
        ...
        data.invoice_details.forEach((detail, detailIndex) => {
            ...
            if (Math.abs(expectedTotal - totalRounded) > 0.01) {
                ...
                setError(`invoice_details.${detailIndex}.total_price` as const, {
                    type: "manual",
                    message: `Moet ${expectedTotal.toFixed(2)} zijn (pre-BTW + pre-BTW* BTW)`,
                });
            }
        });
    ...
    const submitWithValidation = (values: UpdateInvoiceFormValues) => {
        ...
        if (!isValid) {
            // Optioneel: toon gecorrigeerde waarden in een toast, UI-bericht of inline
            console.warn("🟡 Gecorrigeerde Waarden Suggestie:", correctedPrices);
            return;
        }
        onSubmit(values);
    }
            ...
            <form onSubmit={form.handleSubmit(submitWithValidation)} className="space-y-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    ...
                                                    <FormControl>
                                                        <Select onValueChange={field.onChange} defaultValue={field.value} >
                                                            ...
                                                            <SelectContent className="bg-white">
                                                                <SelectGroup>
                                                                    {
                                                                        [{ value: "ambulante", label: "Ambulante" }, { value: "accommodation", label: "Accommodatie" }].filter(v => v.value !== "").map((item, index) => (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>Prijstijdeenheid</FormLabel>
                                                    <FormControl>
                                                        <Input {...field} />
                                                    </FormControl>
                                                    <FormMessage />
                ...
                <div className="flex justify-end gap-4">
                    <Button type="submit" disabled={isSubmitting}>
                        {isSubmitting ? "Opslaan..." : "Factuur Bijwerken"}
                    </Button>
