...
const EditReports: FunctionComponent = () => {
  ...
  const { readOne } = useReport({ autoFetch: false, clientId: +(clientId as string) || 0 });
  ...
  if (!clientId || Array.isArray(clientId) || !reportsId || Array.isArray(reportsId)) {
    return null;
  }
  if (!report) {
    return <Loader />;
      ...
      <div className="grid grid-cols-1 gap-9 sm:grid-cols-2">
        <div className="flex flex-col gap-9">
          ...
          <div className="bg-white border rounded-sm border-stroke shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <ReportsForm mode={"edit"} reportId={+reportsId} clientId={+clientId} report={report} />
          </div>
        </div>
      </div>
    </>
