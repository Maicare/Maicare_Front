...
const ReportsPage: FunctionComponent = () => {
  ...
  const { reports, isLoading,error } = useReport({ clientId: +(clientId as string), autoFetch: true, page });
  const { open } = useModal(
                  ...
                  <div className="group">
                    <div className="text-slate-800  dark:text-white flex gap-4">
                      <div>
                        ...
                        <span className="font-medium">{post?.employee_first_name + " " + post?.employee_last_name}</span>
                      </div>
                    ...
                    <span className="mt-1 block text-sm">
                      {getLocateDatetime(post?.date || "")} - {REPORT_TYPE_RECORD[post?.type as keyof typeof REPORT_TYPE_RECORD]}
                    </span>
