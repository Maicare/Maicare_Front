...
import { CreateSupportNetwork, createSupportNetworkSchema } from '@/schemas/plan-care.schema';
import { Input } from '@/components/ui/input';
type Props = {
    mode: "create" | "update";
    handleCreate: (values: CreateSupportNetwork) => void;
    handleUpdate?: (values: CreateSupportNetwork, id: Id) => void;
    supportNetwork?: SupportNetwork;
    isOpen: boolean;
    handleOpen: (bool: boolean) => void;
}

const UpsertSupportNetworkSheet = ({ mode, handleCreate, handleUpdate, supportNetwork, handleOpen, isOpen }: Props) => {
    const [loading, setLoading] = useState(false);
    const form = useForm<CreateSupportNetwork>({
        resolver: zodResolver(createSupportNetworkSchema),
        defaultValues: supportNetwork ? {
            ...supportNetwork
        } : {
    ...
    async function onSubmit(values: CreateSupportNetwork) {
        setLoading(true);
        if (mode === "create") {
            handleCreate(values);
        } else {
            handleUpdate?.(values, supportNetwork!.support_network_id);
        }
        handleOpen(false);
        setLoading(false);
    }
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className='flex items-center justify-between'>
                                        Rol Titel
                                        <Tooltip text='This is Role Title'>
                                            <Info className='h-5 w-5 mr-2' />
                                        </Tooltip>
                                    </FormLabel>
                                    <FormControl>
...
export default UpsertSupportNetworkSheet