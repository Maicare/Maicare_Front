    ...
    return (
        <div className='flex flex-col gap-4'>
            <div className="grid grid-cols-3 gap-4 w-full">
                <ProfileInformation
                    date_of_birth={client?.date_of_birth || "Niet gespecificeerd"}
                    first_name={client?.first_name || "Niet gespecificeerd"}
                    last_name={client?.last_name || "Niet gespecificeerd"}
                    gender={client?.gender || "Niet gespecificeerd"}
                    isParentLoading={isLoading}
                    profile_picture={client?.profile_picture || "Niet gespecificeerd"}
                    status={client?.status || "Onbekend"}
                />
                <PersonalInformation
                    email={client?.email || "Niet gespecificeerd"}
                    first_name={client?.first_name || "Niet gespecificeerd"}
                    last_name={client?.last_name || "Niet gespecificeerd"}
                    isParentLoading={isLoading}
                    birthplace={client?.birthplace || "Niet gespecificeerd"}
                    private_phone_number={client?.phone_number || "Niet gespecificeerd"}
                />
                <WorkInformation
                ...
                />
                <LocationInformation
                    Zipcode={client?.Zipcode || "Niet gespecificeerd"}
                    departement={client?.departement || "Niet gespecificeerd"}
                    city={client?.birthplace || "Niet gespecificeerd"}
                    street_number={client?.street_number || "Niet gespecificeerd"}
                    streetname={client?.streetname || "Niet gespecificeerd"}
                    location={client?.location || "Niet gespecificeerd"}
                    isParentLoading={isLoading}
                />
                <AddressesPreview
                    isParentLoading={isLoading}
