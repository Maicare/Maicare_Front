...
export function EnhancedDatePicker({ 
  ...
  return (
    <FormField
      ...
      name={name}
      render={({ field }) => {
        // Helper functions that need access to field
        const handleMonthSelect = (monthIndex: number) => {
          const newDate = new Date(currentMonth.getFullYear(), monthIndex, 1);
          setCurrentMonth(newDate);
          
          // If there's a selected date, update its month while preserving day and year
          if (field.value) {
            const updatedDate = new Date(
              field.value.getFullYear(),
              monthIndex,
              Math.min(field.value.getDate(), new Date(field.value.getFullYear(), monthIndex + 1, 0).getDate())
            );
        ...
        const handleYearSelect = (year: number) => {
          ...
          if (field.value) {
            const updatedDate = new Date(
              ...
              Math.min(field.value.getDate(), new Date(year, field.value.getMonth() + 1, 0).getDate())
            );
            field.onChange(updatedDate);
          }
          setYearView(false);
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        onClick={() => {
                          const recentYear = currentYear - 18;
