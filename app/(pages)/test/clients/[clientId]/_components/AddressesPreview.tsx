    ...
    return (
      <Card className="w-full h-full border-2 border-dashed border-gray-200 hover:border-blue-300 transition-colors">
        <CardContent className="p-6 flex flex-col items-center justify-center h-full text-center">
          ...
          <CardTitle className="text-lg mb-2">Adresgegevens</CardTitle>
          <CardDescription className="mb-4">
            Geen adressen gevonden voor deze cliënt
          </CardDescription>
          <Button 
            ...
            <AccordionItem value={`item-${index + 1}`} key={index} className="border-b last:border-b-0 px-6">
              ...
              <AccordionContent className="pb-4 space-y-3">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm font-medium text-gray-500">Behoort toe aan</p>
                    <CopyTooltip text={belongs_to || "Niet gespecificeerd"}>
                      <p className="text-sm text-gray-900 font-medium">{belongs_to || "Niet gespecificeerd"}</p>
                    </CopyTooltip>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Plaats</p>
                    <CopyTooltip text={city || "Niet gespecificeerd"}>
                      <p className="text-sm text-gray-900 font-medium">{city || "Niet gespecificeerd"}</p>
                    </CopyTooltip>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Telefoonnummer</p>
                    <CopyTooltip text={phone_number || "Niet gespecificeerd"}>
                      <p className="text-sm text-gray-900 font-medium">{phone_number || "Niet gespecificeerd"}</p>
                    </CopyTooltip>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Postcode</p>
                    <CopyTooltip text={zip_code || "Niet gespecificeerd"}>
                      <p className="text-sm text-gray-900 font-medium">{zip_code || "Niet gespecificeerd"}</p>
                    </CopyTooltip>
                  </div>
                </div>
              </AccordionContent>
