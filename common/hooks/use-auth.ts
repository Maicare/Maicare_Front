...
export function useAuth({ autoFetch = true }: { autoFetch?: boolean }) {
	...
	const login = async (credentials: LoginInput, options?: ApiOptions) => {
		...
		try {
			...
			const { message, success, data, error } = await useApi<LoginResponse>(
	...
	const logout = async (options?: ApiOptions) => {
		...
		try {
			// Display progress bar
			if (displayProgress) startProgress();
			const { message, success, error } = await useApi<LoginResponse>(
			...
			enqueueSnackbar(err?.response?.data?.message || "Logout failed", {
				variant: "error",
			});
			throw err;
		} finally {
	...
	const changePassword = async (
		...
		try {
			...
			const { message, success, error } = await useApi(
	...
	const setup2FA = async (options?: ApiOptions) => {
		...
		try {
			...
			enqueueSnackbar(err?.response?.data?.message || "2FA setup failed", {
				variant: "error",
			});
			throw err;
		} finally {
	...
	const enable2FA = async (validation_code: string, options?: ApiOptions) => {
		...
		try {
			...
			enqueueSnackbar(err?.response?.data?.message || "2FA enable failed", {
				variant: "error",
			});
			throw err;
		} finally {
