...
export function useCalendar(employeeId:string,autoFetch:boolean=false, params?: PaginationParams) {
    ...
    const {
        ...
        autoFetch ? `${ApiRoutes.Employee.Appointmens.ReadAll.replace(
            "{id}",
            employeeId
        )}` :null,
        async (url) => {
    ...
    const createAppointment = async (appointmentData: CreateAppointmentType, options?: ApiOptions) => {
        ...
        try {
            ...
            const { message, success, data, error } = await useApi<CreateAppointmentType>(ApiRoutes.Employee.Appointmens.CreateOne
